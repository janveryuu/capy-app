...
import { UPCOMING_TRIP } from '@/lib/capy-data'
...
function StatPill({
  ...
  return (
    <div className="flex items-center gap-3 rounded-3xl border border-border bg-card/70 p-4">
...
export function Dashboard({ onGoTravel }: { onGoTravel: () => void }) {
  return (
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-6">
      ...
      >
        <div className="flex flex-col items-center gap-6 sm:flex-row sm:items-center sm:gap-8">
          <CapyMascot size={176} className="shrink-0" />
          ...
          <div className="mt-auto pt-6">
            <div className="rounded-3xl bg-matcha/40 p-4">
              <div className="flex items-end justify-between">
                ...
                <span className="rounded-full bg-card/70 px-3 py-1 text-xs font-bold text-foreground transition-colors group-hover:bg-card">
                  Open trip →
                </span>
              </div>
            </div>
